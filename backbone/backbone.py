...
def csp_block(inputs, filters, num_blocks):
    ...
    for _ in range(num_blocks):
        block_inputs = x
        x = conv_bn(x, x.shape[3], kernel_size=1, strides=1, activation="mish")
        x = conv_bn(x, x.shape[3], kernel_size=3, strides=1, activation="mish")
        x = x + block_inputs
...
def csp_darknet53(input_shape):
    ...
    x = conv_bn(x, filters=64, kernel_size=3, strides=2, zero_pad=True, padding="valid", activation="mish",)
